...
public:
	...
	bool HasComponent() const {
		const char* componentTypeName = typeid(T).name();
		return components.find(componentTypeName) != components.end();
	}

	template<typename T>
	std::vector<T>* TryGetComponents() {
		const char* componentTypeName = typeid(T).name();
		auto pair = components.find(componentTypeName);
		if (pair == components.end())
			return nullptr;
		return &std::static_pointer_cast<ComponentCollection<T>>(pair->second)->GetComponents();
	}
